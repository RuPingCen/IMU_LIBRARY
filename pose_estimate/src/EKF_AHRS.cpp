...
// 根据加速度的观测值，如果模长大于1 就增加R矩阵的特征值，降低加速度观测的权重
bool EKF_AHRS::Run2(const double& dt,const Eigen::Vector3d& gyro,
			const Eigen::Vector3d& acc,const Eigen::Vector3d& mag)
{
  ...
  //cout<< "r_temp: " << r_temp.transpose()<<endl;
	...
	{
		tem_Kp = (acc_norm*acc_norm); 
		cout<<"acc_norm: "<<acc_norm<<"\ttem_Kp: "<<tem_Kp<<endl;
	}

	Eigen::MatrixXd S = H_thin*P*H_thin.transpose() + R*tem_Kp;
	Eigen::MatrixXd K = P*H_thin.transpose()*S.inverse();
