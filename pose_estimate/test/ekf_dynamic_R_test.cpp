/**
 * @function 读取 Xsens 的数据 利用加速度的观察信息 动态调整 EKF 观测矩阵的值 
 *      实现变权重的姿态融合算法，并对比原始的EKF的算法
 * maker:crp
 * 2020-11-6
 */
...
int main(int argc,char** argv)
{
	...
	ros::init(argc, argv, "ekf_dynamic_test");
	ros::NodeHandle n("~");
	...
	debug_pub = n.advertise<sensor_msgs::Imu>("/pose_estimate/ekf_static_R/imu", 10);
	debug_pub2= n.advertise<sensor_msgs::Imu>("/pose_estimate/ekf_dynamic_R/imu", 10);
	...
	IMU::EKF_AHRS ekf_static_R;
	IMU::EKF_AHRS ekf_dynamic_R;
    ...
    { 
		 if(use_mag == true)
		 {
			if((imu.IMUFlag&0x01) && (imu.IMUFlag&0x02) && (imu.IMUFlag&0x04))
			{
				...
				ekf_static_R.Run(dt,gyro,acc,mag);
				ekf_dynamic_R.Run2(dt,gyro,acc,mag);
			  
			} 
		}
		 

		Eigen::Vector4d dq_imu = ekf_static_R.getQuaternion();
		...
		Eigen::Vector4d dq_imu_dynamic_kp= ekf_dynamic_R.getQuaternion();
